...
export default function Sources() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-3 gap-5">
      <div className="lg:col-span-2 space-y-5">
        <Card
          title={`Crawled results · ${rows.length}`}
          padded={false}
          action={
            <select
              value={material}
                ...
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-5 py-8 text-center text-ink-500 text-sm">
                      No v1 crawl results (org 0 sentinel). Submit a URL below.
                    </td>
                  </tr>
                )}
                {rows.map((r) => (
